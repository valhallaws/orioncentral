...
<div class="space-y-1">
    ...
    <select
        ...
        name="{{ $name }}"
        {!! $attributes->merge([
            'class' => 'block w-full px-3 py-1.5 rounded-md text-secondary-900 bg-white border border-secondary-300 focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/50 sm:text-sm transition-colors duration-200 disabled:opacity-50 disabled:bg-secondary-50 dark:bg-secondary-900 dark:border-secondary-700 dark:text-neutral-100 dark:focus:ring-primary-500/50 dark:disabled:bg-secondary-800'
        ]) !!}
    >
        @if(count($options) === 0)
            {{ $slot }}
